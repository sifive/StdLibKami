...
Section Proofs.
 ...
 Goal GenericFifoCorrect fifoImpl fifoSpec.
      ...
      + repeat (repeat goal_split; repeat goal_body; repeat normal_solver).
        ...
        -- do 4 f_equal.
           ...
           ** simpl.
              ...
              clear H16 HdoUpdRegsR HdoUpdRegsR0.
              destruct wltu eqn:G0; [rewrite wltu_lt in G0| rewrite wltu_ge in G0];
                destruct wltu eqn:G1; auto.
              --- rewrite wltu_ge in G1.
                  destruct implRegValL; simpl in *; try lia.
                  rewrite wordToNat_natToWord in G0, G1; try lia;
                    unfold lgSize, size;
                    apply (Nat.le_lt_trans _ (2^ (Nat.log2_up sizeL)));
                    try (apply Nat.pow_lt_mono_r; lia);
                    apply (Nat.le_trans _ sizeL); try lia; apply log2_up_pow2.
              --- rewrite wltu_lt in G1.
                  destruct implRegValL; simpl in *; try lia.
                  destruct (le_lt_or_eq _ _ G0).
                  +++ exfalso.
                      rewrite wordToNat_natToWord in H0, G1; try lia;
                        unfold lgSize, size;
                        apply (Nat.le_lt_trans _ (2^ (Nat.log2_up sizeL)));
                        try (apply Nat.pow_lt_mono_r; lia);
                        apply (Nat.le_trans _ sizeL); try lia; apply log2_up_pow2.
                  +++ rewrite <- H0, wordToNat_natToWord; auto.
                      rewrite wordToNat_natToWord in G0, G1; try lia;
                        unfold lgSize, size;
                        apply (Nat.le_lt_trans _ (2^ (Nat.log2_up sizeL)));
                        try (apply Nat.pow_lt_mono_r; lia);
    ...
    - hyp_consumer.
      ...
      rewrite (Eqdep.EqdepTheory.UIP_refl _ _ x1) in *.
      destruct lt_dec.
      + rewrite evalExpr_castBits, nat_cast_eq_rect.
        unfold size at 2 4; simpl.
        rewrite (f_equal_dep (fun n : nat => word n)
                             (fun n =>
                                (wconcat (wzero (Nat.log2_up size + 1
                                                                    - (Nat.log2_up sizeL + 1))) (if wltu x0 $ (sizeL - Datatypes.length implRegValL)
        then x0
        else $ (sizeL - Datatypes.length implRegValL))))
